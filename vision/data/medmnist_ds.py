...
class MedMNIST2D(MedMNIST):
    ...
    def montage(self, length=20, replace=False, save_folder=None):
        ...
        if save_folder is not None:
            ...
            montage_img.save(os.path.join(save_folder, f"{self.flag}_{self.split}_montage.jpg"))
...
class MedMNIST3D(MedMNIST):
    ...
    def montage(self, length=20, replace=False, save_folder=None):
        ...
        montage_frames = montage3d(imgs=self.imgs, n_channels=self.info["n_channels"], sel=sel)

        if save_folder is not None:
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)

            save_frames_as_gif(montage_frames, os.path.join(save_folder, f"{self.flag}_{self.split}_montage.gif"))
